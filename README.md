# 嗷呜点歌机

<p align="center">
  <img src="docs/images/awoo-musicbot-1.1.5.png" alt="嗷呜点歌机控制面板与 OBS 展示页面" width="100%">
</p>

嗷呜点歌机是一款面向 Bilibili 主播的 Windows 弹幕点歌工具。1.1.5
开始支持网易云音乐、酷狗音乐、QQ 音乐和 Folia，并把各播放器的搜索、
状态读取和控制实现拆成可独立更新的连接器。

项目仓库和底层包标识使用英文名 `AwooMusicBot` / `awoo-musicbot`，安装后的
程序、窗口、快捷方式及其它面向用户的名称均为“嗷呜点歌机”。

## 发布通道

- **嗷呜点歌机 1.1.x / 1.2.x**：推荐的新架构。支持四种播放器、游客模式、
  独立连接器、HTTP/WebSocket 只读接口和内置问题反馈；功能更多、体验
  更好，但仍可能遇到播放器私有协议变化带来的兼容性或稳定性问题。
- **BiliNCM 1.0.x**：旧稳定通道。功能较少，主要面向原有网易云流程，
  但经过更长时间验证。1.0.x 只接收同系列修复，不会被自动升级到嗷呜点歌机通道。

两个通道通过 `https://app.enkianss.us` 独立检查和下载更新。

## 主要功能

- 1.2.1 更新控制面板与置顶窗口界面，采用蓝色主色，默认使用夜间深色主题，并可切换浅色。
  主题选择会保存并在两个窗口间同步；置顶窗口的窗口操作按钮常驻显示，
  队列操作支持鼠标与键盘。界面使用原始应用图标，文案精简为功能名称和状态提示。
  每个播放器连接器均可单独“检查更新”，顶部“检查全部”可刷新全部连接器的版本信息。
- 1.2.1 配合 QQ 音乐连接器 22.61.2，默认使用 QQMusicApi 导出的 Web/API
  连接。下一首由连接器登记和接管，本体不再发送重复纠正；忙碌时返回的旧快照
  不会消耗待播队列。立即播放仍需观察到实际歌曲，操作回执不等于播放成功。
  暂停自动播放会停止新的自动操作并确认逻辑下一首已取消，保留本地点歌队列，
  不发送 QQ 暂停或切歌；取消未确认时界面会提示，恢复后按当前队首重新登记。
  这项适配减少对旧私有偏移的依赖，后续 QQ 更新仍需验证兼容性。
- B站弹幕点歌、撤回、置顶、插队、切歌与分级冷却。
- 游客模式可直接连接直播间并使用普通点歌和撤回；扫码登录仅用于超级
  用户白名单及自定义权限控制。
- 所有弹幕指令先进入有界队列，再严格逐条处理，避免并发搜索、插队和
  队列修改互相冲突。
- 1.1.8 起可以开启 60 秒礼物学习模式，从直播间实际礼物事件中读取并缓存
  礼物名称与 ID，再一键分配给普通观众、舰长、提督或总督的点歌次数规则；
  不保存送礼人的账号信息。
- 四个播放器连接器按需自动安装并在后台检查更新；更新包经过
  SHA-256 与 Ed25519 签名校验，健康检查通过后才会激活。
- 1.1.9 起更新或重连播放器时会等待连接器完成后台控制清理；只有超过
  安全期限仍未退出才会强制结束并记录提示，避免 QQ 音乐首次使用新连接器
  时撞上尚未完成的原生控制事务。
- 1.1.10 起连接器源码仓库采用 `awoo-connectors` 名称；新客户端使用只含
  Awoo framework-dependent 包的 v2 清单，签名信任标识保持不变；旧版点歌机
  继续使用冻结的 v1 清单和历史资产。
- 1.1.10 起内部控制服务仍优先使用 5555，但端口被占用时会稳定回退到
  其它空闲端口；控制面板、内部 API 和经典 OBS 地址都会使用同一个实际
  端口。也可以在“基础设置”中手动指定端口，重启点歌机后生效。
- 1.1.11 起优先完成透明点歌窗口首屏渲染，再启动播放器连接器、直播间与
  外部接口，缩短冷启动时的等待；常见问题页会先引导更新播放器、连接器和
  点歌机。私有 .NET Runtime 的 Range 大小探测被网络或安全软件拒绝时，会
  自动改用流式完整下载，并继续执行官方 SHA-512 校验。
- 1.1.12 起本体自动更新采用单任务保护；重复点击或重复请求只会继续监视
  当前更新，不会再次启动安装器，并且只执行一次最终重启。QQ 音乐在尚未建立
  播放锚点时会保留点播请求，首次开始播放后立即安全接管下一首。悬浮点歌窗新增
  可保存的置顶开关，技术问题反馈会一并提交复现后的脱敏日志。正式版默认单实例，
  开发版可与正式版并行测试。
- 1.1.13 起“运行状态”会明确显示直播间正在连接、弹幕已连接、连接失败或
  已断开，并同时展示输入的房间号和解析后的真实房间号；用户可主动断开，重启后
  不会被旧的重连任务再次连接。登录账号连接其它主播的直播间时会优先使用带签名的
  B站弹幕鉴权，并保留兼容回退。播放器已运行但连接失败时，点歌机会按实际播放器
  版本查找明确匹配的新连接器，最多自动升级并重试一次；没有匹配版本时会提示可能
  缺少新版本连接器，不会仅凭宽泛版本范围盲目更新。
- 1.1.3 起优先使用小体积的框架依赖连接器，并按 x86/x64 架构一次性安装
  点歌机私有的共享 .NET 8 Runtime。1.1.3–1.1.9 在 Runtime 或小包失败时会
  回退 SelfContained 完整包；1.1.10 起只安装小包，不再额外下载约 35 MB 的
  完整兼容包，失败时会保留原连接器并给出明确错误。旧版点歌机需要的完整
  包仍保留在历史 Release 和 v1 清单中。1.1.10 起使用只含 Awoo
  framework-dependent 包的 v2 清单；未来连接器 Release 只包含该 ZIP、签名
  和哈希三个资产；v2 暂不可用时不会静默回退到 v1。
- 播放器切换后自动启动对应连接器并持续探测连接状态。
- 点歌机只把本地待播队列的第一首预插入播放器。该歌曲真正开始播放后，
  才会插入下一首；后续歌曲不会提前批量写入播放器队列。
- 四个连接器均保留错误下一首守卫：如果播放器实际切到的歌曲与本地队首
  不同，会按对应播放器的守卫策略接管目标歌曲；QQ Web 连接器负责自身的
  逻辑下一首，本体不会对同一次未确认操作自动重试。
- QQ 音乐会把同歌手、同基础曲名的“纯音乐”“Inst.”与“Instrumental”
  视为同一首明确的伴奏版本；Live、Remix 等其它版本仍保持严格区分。
  未确认的立即播放动作超时后会恢复播放器实际状态，不再让 UI 误显示点歌仍在播放。
- 可选择展示主播歌单当前歌曲，并可选择点歌人头像或歌曲封面。
- 可选的本机只读 HTTP 与 WebSocket 接口，供 OBS、插件或其它本地工具
  实时读取当前歌曲和待播队列。
- 1.1.5 起支持可安装、切换的 OBS Mod UI：可以识别 GitHub 仓库，也可以
  选择或拖入本地 ZIP；切换 UI 后 OBS 浏览器源地址保持不变。
- “运行状态”会显示播放器当前歌曲、当前启用的 Mod UI，并可直接跳转到
  Mod 设置。内置 UI 和官方示例均采用透明组件布局。

## 快速开始

1. 启动需要使用的播放器，并确保它可以正常播放歌曲。
2. 启动点歌机，在“基础设置”中选择播放器。
3. 第一次选择某个播放器时，点歌机会自动下载并安装对应连接器；不需要
   为网易云添加调试端口，也不会为此重启播放器。
4. 在“运行状态”填写 B站直播间房间号并连接。扫码登录不是必需步骤。
5. 在 OBS 或直播姬中添加浏览器源。经典页面和 Mod UI 的捕捉地址都以
   “外观设置”或“基础设置”中显示的实际地址为准：经典页面默认使用内部
   端口 5555，Mod UI 默认使用外部只读端口 5556；端口冲突自动回退后不要
   继续照抄默认值。

连接器是播放器协议的适配层。播放器更新后，如果没有新增本体协议或行为依赖，
只需更新对应连接器。QQ 音乐连接器 22.61.2 依赖本体的逻辑队列、观察和自动播放
控制适配，需要嗷呜点歌机 1.2.1 或更新版本。

## 弹幕指令

| 指令 | 功能 | 游客模式 |
|---|---|---|
| `点歌 歌名或ID` | 搜索并加入待播队列 | 可用 |
| `撤回` | 删除自己最近点的一首歌 | 可用 |
| `置顶点歌 歌名` | 放到本地待播队首 | 需登录并满足权限 |
| `插队点歌 歌名` | 中断当前歌曲并插播 | 需登录并满足权限 |
| `立即点歌 歌名` | 立即播放目标歌曲 | 需登录并满足权限 |
| `切歌` / `跳过` | 跳过当前歌曲 | 需登录并满足权限 |
| `开始接单` / `停止接单` | 切换接单状态 | 需登录并满足权限 |

不同播放器使用各自的直接点歌格式：

- 网易云和 Folia：`id=1403356922` 是明确歌曲 ID；6 位以上纯数字是疑似
  ID，会与同文本关键词搜索并行，精确 ID 存在时优先，否则使用关键词结果。
- 酷狗：支持纯数字分享码、`#17154574#`、`chain=ig6N5bG4V3`、裸 chain
  及完整 `m.kugou.com/share/song.html?chain=...` 链接；精确解析失败时回退关键词。
- QQ 音乐：支持完整 `c6.y.qq.com/base/fcgi-bin/u?__=...` 分享链接、
  `u?__=0eBs266kH6Oj` 和裸 12 位分享码；普通纯数字保持关键词搜索。

## 本地只读接口

在“基础设置”中可以分别启用 HTTP API 和 WebSocket 推送，并设置监听端口。
接口只绑定 `127.0.0.1`，默认端口为 `5556`。

- `GET /health`
- `GET /api/v1/state`
- `GET /api/v1/current`
- `GET /api/v1/queue`
- `ws://127.0.0.1:5556/ws`
- OBS 浏览器源示例：`http://127.0.0.1:5556/overlay/`

接口只提供播放与队列状态，不接受控制命令。
完整字段、兼容规则和第三方前端接入方式见
[`docs/external-api.md`](docs/external-api.md)；可直接修改的零构建示例源码位于
[`examples/obs-overlay`](examples/obs-overlay)。

## OBS Mod UI

在“运行状态 → Mod UI”中可以：

- 输入 GitHub 仓库或发布清单网址，识别并安装 UI。
- 选择本地 ZIP，或将 Mod UI ZIP 拖到安装区域。
- 查看当前使用的 UI，并在已安装的 UI 之间切换。

OBS 始终使用同一个 `/overlay/` 地址，安装或切换 UI 后不需要修改浏览器源。
第三方 UI 的清单、ZIP 结构和安全限制见
[`docs/overlay-mods.md`](docs/overlay-mods.md)。官方示例仓库为
[`Enkianssus/AwooMusicBot-Overlay-Default`](https://github.com/Enkianssus/AwooMusicBot-Overlay-Default)。

## 本地开发

版本号、连接器签名资产、GitHub Actions 上传、Cloudflare 部署和旧版兼容的
完整规范见 [`docs/RELEASING.md`](docs/RELEASING.md)。

```powershell
npm ci
npm test
npm run lint
npx tsc --noEmit
npm run build:dev
```

连接器位于 `BiliNCM-Connectors` 子项目：

```powershell
dotnet build .\BiliNCM-Connectors\BiliNCM.Connectors.slnx -c Release
```

## 技术栈

- Electron、Node.js、React、Vite、TypeScript、Tailwind CSS
- .NET 8 Windows x86/x64 独立播放器连接器与私有共享 Runtime
- Bilibili 原生弹幕 WebSocket
- Velopack、GitHub Actions、Cloudflare 下载代理

## 许可

本项目基于 MIT License。使用时仍需遵守 Bilibili 及各音乐平台的服务条款。
