...
Awoo MusicBot 是一款面向 Bilibili 主播的 Windows 弹幕点歌工具。1.1.0
开始支持网易云音乐、酷狗音乐、QQ 音乐和 Folia，并把各播放器的搜索、
状态读取和控制实现拆成可独立更新的连接器。

## 发布通道

- **Awoo MusicBot 1.1.x**：推荐的新架构。支持四种播放器、游客模式、
...
ID 识别与普通关键词搜索并行执行：网易云和酷狗支持纯数字候选，QQ 音乐
支持分享短链、`id=` 前缀以及 12 位数字字母候选，Folia 支持网易云歌曲 ID。
ID 无效时会自动回退普通搜索。
