...
const OverlayWidget: React.FC<OverlayWidgetProps> = ({ onToggleAdmin }) => {
    ...
    useEffect(() => { localStorage.setItem('bili-widget-theme', JSON.stringify(theme)); }, [theme]);
                    ...
                    if (json.uiConfig.timestamp > lastSyncTimeRef.current) {
                        ...
                        if (!isElectron) {
                            setWidgetStyle(prev => {
                                ...
                                if (bPos && typeof bPos.x === 'number') {
                                    next.x = bPos.x; next.y = bPos.y;
                                    localStorage.setItem('bili-widget-pos', JSON.stringify(bPos));
                                    changed = true;
                                }
    ...
    const handlePointerDown = (e: React.PointerEvent<HTMLDivElement>, type: 'current' | 'queue', index: number, item: SongInfo) => {
        ...
        const onPointerMove = (ev: PointerEvent) => setDragInfo(prev => prev ? { ...prev, x: ev.clientX, y: ev.clientY } : null);
        const onPointerUp = (ev: PointerEvent) => {
            ...
            const isQueueZone = dropTarget.closest('.queue-zone');
            const isCurrentZone = dropTarget.closest('.current-zone');

            if (type === 'queue') {
                if (isCurrentZone) handleQueueAction('play_now', { index });
                else if (isQueueZone) {
                ...
                } else { handleQueueAction('delete', { index }); }
            } else if (type === 'current') {
                if (isQueueZone) {
                    handleQueueAction('push_current_to_queue', {});
                }
                else if (!isCurrentZone) handleQueueAction('skip_current', {});
            }
        };
    ...
    const handleWindowClose = () => {
        if (isElectron && ipcRenderer) {
            ...
            fetch('http://localhost:5555/api/exit', { method: 'POST' });
            window.close();
        }
    };
    ...
    const getStatusColor = (status: string | undefined) => {
        ...
        if (status.includes('❌') || status.includes('🔴')) return '#f87171'; // Red
        if (status.includes('✅') || status.includes('🟢')) return '#4ade80'; // Green
        if (status.includes('⚠️') || status.includes('拦截')) return '#facc15'; // Yellow
        if (status.includes('⬆️') || status.includes('⏭️') || status.includes('🔙') || status.includes('🔄') || status.includes('▶️') || status.includes('⚡')) return '#c084fc'; // Purple
        if (status.includes('test') || status.includes('测试')) return '#22d3ee'; // Cyan
    ...
    return (
        <div className={isElectron
            ...
            {dragInfo && (
                <div
                    className="no-drag fixed z-[999999] glass-card rounded-xl p-2.5 flex items-center gap-3 opacity-95 shadow-[0_20px_50px_rgba(0,0,0,0.6)] scale-105 pointer-events-none"
                    style={{ left: dragRenderX, top: dragRenderY, transform: 'translate(-50%, -50%)', width: '260px', background: 'rgba(20,20,20,0.7)', backdropFilter: 'blur(16px)' }}
                >
                ...
                {isElectron && showSettings && (
                    ...
                    >
                        ...
                        <div className="mt-auto flex flex-col gap-2">
                            <button className="py-2.5 w-full bg-blue-500 hover:bg-blue-400 rounded-lg text-white text-xs font-bold transition-colors shadow-[0_0_10px_rgba(59,130,246,0.5)] border border-blue-400" onClick={saveToGlobal}>保存并同步全局</button>
...
const AdminWidget: React.FC<AdminWidgetProps> = ({ onClose: _onClose }) => {
    ...
    useEffect(() => {
        const fetchConfig = async () => {
            try {
                ...
                setConfig((prev: any) => {
                    if (!prev || activeTabRef.current !== 'settings') return json;
                    return prev;
                });
    ...
    const saveConfig = async (newCfg: any) => {
        await fetch('http://localhost:5555/api/config', {
            method: 'POST',
            headers: {'Content-Type':'application/json'},
                            ...
                            {activeTab === 'settings' && (
                                <div className="animate-slide-in-right pb-20">
                                    ...
                                    <div className="bg-white/5 p-6 rounded-xl border border-purple-500/40 space-y-5 mb-6 shadow-[0_0_15px_rgba(168,85,247,0.15)] relative overflow-hidden">
                                        ...
                                        <h3 className="text-sm font-bold text-purple-400 uppercase tracking-widest border-b border-white/10 pb-3 flex justify-between items-center">
                                            <span>💻 客户端 JS 注入 (CDP 调试)</span>
                                        </h3>

                                        <div className="flex flex-col gap-5">
                                            <div className="flex justify-between items-center bg-black/40 border border-white/10 rounded-lg p-4">
                                                <div>
                                                    <label className="block text-sm text-white font-bold mb-1">开启网易云 JS 注入操控 (CDP)</label>
                                                    <span className="text-xs text-gray-400 block max-w-lg leading-relaxed">开启后，点歌机将不再使用按键模拟，而是通过底层 JS直接操控网易云 Redux Store。更稳定，切歌更静默！</span>
                                                </div>
                                                <button onClick={() => setConfig({...config, config: {...config.config, EnableCDP: !config.config.EnableCDP}})} className={`w-12 h-6 rounded-full p-1 transition-colors shrink-0 ${config.config.EnableCDP ? 'bg-purple-600' : 'bg-gray-600'}`}>
                                                    <div className={`w-4 h-4 rounded-full bg-white transition-transform ${config.config.EnableCDP ? 'translate-x-6' : 'translate-x-0'}`}></div>
                                                </button>
                                            </div>

                                            <div className="flex gap-4">
                                                <div className="flex-1">
                                                    <label className="block text-xs text-gray-400 mb-2">CDP 调试端口 (默认: 9222)</label>
                                                    <input type="number" className="w-full bg-black/30 border border-white/10 rounded-lg p-3 text-md text-white focus:border-purple-500 outline-none" value={config.config.CdpPort || 9222} onChange={e => setConfig({...config, config: {...config.config, CdpPort: parseInt(e.target.value)}})} />
                                                </div>
                                                <div className="flex flex-col justify-end">
                                                    <button onClick={handleRestartNCM} className="px-6 py-3 bg-purple-600 hover:bg-purple-500 text-white text-md rounded-lg font-bold shadow-lg transition-colors border border-purple-400/50 flex items-center gap-2">
                                                        🔌 强制带调试端口重启网易云
                                                    </button>
                                                </div>
                                            </div>
                                    ...
                                    <div className="fixed bottom-0 left-40 right-0 bg-[#0d1117]/95 backdrop-blur-xl border-t border-white/10 p-5 flex justify-end z-20">
                                        <button onClick={() => saveConfig(config.config)} className="px-8 py-3 bg-blue-600 hover:bg-blue-500 text-white text-md rounded-xl font-bold shadow-[0_0_20px_rgba(37,99,235,0.4)] transition-all transform active:scale-95">
                                            💾 保存并应用所有设置
                                        </button>
                                    </div>
                                </div>
                            ...
                            {activeTab === 'login' && (
                                <div className="space-y-6 animate-slide-in-right flex flex-col items-center pt-10">
                                    ...
                                    <div className="bg-white/5 p-8 rounded-2xl border border-white/10 flex flex-col items-center justify-center w-full max-w-md text-center shadow-xl">
                                        ...
                                        <p className="text-xs text-gray-500 mt-4">完全在本地完成登录，仅获取基础弹幕权限。</p>
                                    </div>
                                </div>
                            )}
